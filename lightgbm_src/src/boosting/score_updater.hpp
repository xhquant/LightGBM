    ...
    public:
        ...
        */
        ScoreUpdater(const Dataset *data, int num_tree_per_iteration) : data_(data)
        {
            ...
            {
                if ((data->metadata().num_init_score() % num_data_) != 0 || (data->metadata().num_init_score() / num_data_) != num_tree_per_iteration)
                {
...
#pragma omp parallel for schedule(static, 512) if (total_size >= 1024)
        ...
        inline bool has_init_score() const
        {
...
#pragma omp parallel for schedule(static, 512) if (num_data_ >= 1024)
        ...
        */
        ...
        virtual inline const double *score() const
        {
            return score_.data();
        }

        inline data_size_t num_data() const
        {
            return num_data_;
        }
