...
yamc::shared_lock<yamc::alternate::shared_mutex> lock(&mtx);
    ...
    public:
        ...
        SingleRowPredictor(int predict_type, Boosting *boosting, const Config &config, int start_iter, int num_iter)
        {
            ...
            iter_ = num_iter;
            predictor_.reset(new Predictor(boosting, start_iter, iter_, is_raw_score, is_predict_leaf, predict_contrib, early_stop_, early_stop_freq_, early_stop_margin_));
    ...
    public:
        ...
        Booster(const Dataset *train_data, const char *parameters)
        {
            ...
            boosting_->Init(&config_, train_data_, objective_fun_.get(), Common::ConstPtrInVectorWrapper<Metric>(train_metric_));
        }
        ...
        void ResetTrainingData(const Dataset *train_data)
        {
            if (train_data != train_data_)
            {
                ...
                CreateObjectiveAndMetrics();
                // reset the boosting
                boosting_->ResetTrainingData(train_data_, objective_fun_.get(), Common::ConstPtrInVectorWrapper<Metric>(train_metric_));
            }
        }
        ...
        static void CheckDatasetResetConfig(const Config &old_config, const std::unordered_map<std::string, std::string> &new_param)
        {
            ...
            if (new_param.count("data_random_seed") && new_config.data_random_seed != old_config.data_random_seed)
            {
            ...
            {
                Log::Fatal("Reducing `min_data_in_leaf` with `feature_pre_filter=true` may "
            ...
            if (new_param.count("precise_float_parser") && new_config.precise_float_parser != old_config.precise_float_parser)
            {
        ...
        void AddValidData(const Dataset *valid_data)
        {
            ...
            boosting_->AddValidDataset(valid_data, Common::ConstPtrInVectorWrapper<Metric>(valid_metrics_.back()));
        }
        ...
        {
            UNIQUE_LOCK(mutex_)
            if (single_row_predictor_[predict_type].get() == nullptr || !single_row_predictor_[predict_type]->IsPredictorEqual(config, num_iteration, boosting_.get()))
            {
                single_row_predictor_[predict_type].reset(new SingleRowPredictor(predict_type, boosting_.get(), config, start_iteration, num_iteration));
            }
        }

        void PredictSingleRow(int predict_type, int ncol, std::function<std::vector<std::pair<int, double>>(int row_idx)> get_row_fun, const Config &config, a double *out_result, int64_t *out_len) const
        {
        ...
        Predictor CreatePredictor(int start_iteration, int num_iteration, int predict_type, int ncol, const Config &config) const
        {
            ...
            return Predictor(boosting_.get(), start_iteration, num_iteration, is_raw_score, is_predict_leaf, predict_contrib, config.pred_early_stop, config.pred_early_stop_freq, config.pred_early_stop_margin);
        }

        void Predict(int start_iteration, int num_iteration, int predict_type, int nrow, int ncol, std::function<std::vector<std::pair<int, double>>(int row_idx)> get_row_fun, const Config &config, double *out_result, int64_t *out_len) const
        {
            SHARED_LOCK(mutex_);
